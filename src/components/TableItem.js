  ...
  return (
    <>
      <div className="table-card">
        <Link className="table-card-left-wrapper" to={`/vehicle/${item?.vin}`} onClick={newLink}>
          <div className="table-card-image-wrapper">
            <img className="table-card-image" src={item?.imageURL !== "" ? item?.imageURL : NoImage} alt="grid" />
          </div>
        ...
        <div className="table-card-right-wrapper">
          <div className="table-price">${item?.price?.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</div>
          {/*selected?.wishlist ? (
            <button
