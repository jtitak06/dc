    ...
    return (
      <>
        <div className="grid-card">
          <Link className="grid-image-wrapper" to={`/vehicle/${item.vin}`} onClick={newLink}>
            <img className="grid-image" src={item?.imageURL !== "" ? item?.imageURL : NoImage} alt="grid" />
          </Link>
          <div className="grid-info-section">
            ...
            <ul className="grid-list-wrapper">
              <li className="grid-list-item">
                <p className="grid-price">${item?.price.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</p>
              </li>
              {/*<li className="grid-list-item">
                {selected?.wishlist ? (
                  <button
